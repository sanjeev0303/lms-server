...
export const ExpressApp = async (): Promise<Application> => {
    const app = express()
  ...
  app.use(compression({
    level: env.NODE_ENV === 'production' ? 6 : 1, // Higher compression in production
  ...
  app.use(cors({
    ...
    maxAge: env.NODE_ENV === 'production' ? 86400 : 0 // Cache preflight for 24h in production
  }));
  ...
  app.use(morgan(morganFormat, {
    ...
    // WEBHOOK: Raw body for webhook signature verification (must be before JSON parsing)
  ...
  app.use(express.json({
    limit: env.NODE_ENV === 'production' ? '10mb' : '50mb',
